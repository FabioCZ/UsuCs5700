...
{
    class MotherPersonMatcher : PersonMatcher
    {
        protected override Predicate<Person> GetPredicate(Person per)
        {
            return p =>
            {
                ...
                if (IsNullOrEmpty(pChild.MotherFirstName) || IsNullOrEmpty(perChild.MotherFirstName)
                    ...
                    || pChild.BirthOrder == null || pChild.BirthOrder < 1 || perChild.BirthOrder == null || pChild.BirthOrder < 1)
                {
                    return false;
                }

                return pChild.MotherFirstName == perChild.MotherLastName &&
